...
// Source files for platform specific stuff, screen funcs and keyboard handling
#include "system/platform.c"
...
void logic_run()
{
	...
	// if the movement buffer isn't empty
	if (movement_buffer[0] != MOVE_NONE)
	{
		// set the snake's direction based on earliest buffered action
	...
	if (food.x == snk.pos.x && food.y == snk.pos.y)
	{
		...
		// If the snake is below 25 length, add speed.
		if (snk.len < 25) snk.spe += 1;
		// If the snake is +25 and -50, add speed at half rate.
		else if (snk.len < 50)
		{
			if (snk.spe % 2) snk.spe += 1;
		}
		// Once the snake is 50 length, add no more speed, it'll be hard enough.
...
{
	// check collision on self, for every "bone" of the snake's body...
...
void wall_check()
{
	// if the snake runs into a wall, move it to the other end.
	if (snk.pos.x <= 1)
